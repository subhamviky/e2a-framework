...
| io_config | INPUT | Resolved in BaseWorkflow.execute() from the IO_CONFIG namespace | Separates connection/endpoint configuration from behavioral NFR configuration. Also carries max_token_budget, read by BaseGovernanceFramework.enforce_governance_gate() (Implementation Playbook Section 2.9.2) to cap cumulative LLM spend per request. |
| message_log | OUTPUT | Empty list at request start; every class appends structured entries | One ordered, structured log per request. BaseWorkflow.execute() hands it to a configured BaseObservability.record_telemetry() (Section 2.9.1) in its finally block, falling back to per-entry logging otherwise. BaseAgent.run(), BaseRAGPipeline.retrieve(), and BaseToolService.execute() each attach their own latency (seconds) to their completion entry, and BaseAgent.run() additionally attaches tokens_used and confidence. |
...
| Observability / BaseObservability | Logging, metrics, and tracing stack, cross-cutting | Collects telemetry from every tier, correlated by correlation_id, via BaseObservability.record_telemetry() (Implementation Playbook Section 2.9.1) when an observability_engine is configured; falls back to per-entry logging otherwise |
| Governance / BaseGovernanceFramework | Orchestration Compute (Tier 2, Private VPC) | Token-budget enforcement, semantic-firewall redaction, MCP sandbox verification, and human-in-the-loop dehydration, via BaseGovernanceFramework.enforce_governance_gate() (Implementation Playbook Section 2.9.2) when a governance_engine is configured; invoked from BaseAgent.run() in place of _apply_policy() |
...
Each stage's ceiling above is an infrastructure-level budget, not a code-level enforcement mechanism on its own: the actual per-call ceiling is enforced inside the class that does the work. BaseAgent.run(), BaseRAGPipeline.retrieve(), and BaseToolService.execute() (Implementation Playbook Sections 2.3, 2.5, 2.6) each measure their own wall-clock latency against a configured max_latency and raise NFRViolationError on breach — 2.0s by default for the agent and RAG paths, 5.0s for the tool-call path, tunable per deployment via the MAX_LATENCY environment variable or per-call override. A breach is treated as an ordinary failure: the item's key is added to failed_keys and routed through the same DLQ path as any other error (Section 7), not as a special case.
...
## 8.5 Token Economics & Governance Enforcement


Token spend is a per-request running total, not a separate infrastructure metric: state['cumulative_tokens_used'] accumulates across every BaseAgent.run() call in a request chain, incremented by the private __track_usage() helper (Implementation Playbook Section 2.3) after each LLM call — from the provider's own reported usage where available, or a ~4-chars/token estimate otherwise. BaseGovernanceFramework.enforce_governance_gate() (Section 2.9.2) checks that running total against io_config['max_token_budget'] before the next call starts, raising NFRViolationError once the budget is exhausted — the same exception type, failure path, and DLQ routing as a latency breach (Section 8.3), so no separate cost-control infrastructure is needed beyond the existing Tier 2 compute and DLQ.
...
- RAG evaluation gate: BasePipeline._run_rag_eval() (Playbook Section 2.9.3, Foundation Classes) blocks promotion if faithfulness/groundedness drops below threshold.
...
| Resilience | Token-budget enforcement, semantic-firewall PII/prompt-injection redaction, MCP sandbox verification, and circuit breakers with jittered retries, all via BaseGovernanceFramework.enforce_governance_gate() (Implementation Playbook Section 2.9.2); DLQ isolation; Saga compensation |
| Cost Optimization | Serverless Tier 1 avoids paying for Tier 2 compute on invalid requests; Tier 4 scales to zero; managed messaging over self-hosted; a human-in-the-loop approval pause dehydrates state to the Data & Messaging Perimeter and releases Tier 2 compute entirely while awaiting the approval webhook (BaseGovernanceFramework, Section 2.9.2), rather than holding a paid task open on a blocking wait |
