...
**Governance injection points:** `BaseOrchestrator.execute()`, before routing to either service — `enforce_governance_gate()` runs the circuit breaker and semantic firewall against the inbound payload, identically for Command and Query intents. Dehydration (human-in-the-loop halt) is retained as an escape hatch for commands that cross a policy threshold (e.g. a refund above a configured amount).
