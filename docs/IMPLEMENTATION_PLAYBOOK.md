...
| max_latency | MAX_LATENCY | 2.0 | run() — latency SLO measured end-to-end, raises NFRViolationError on breach |
...
| PRIVATE | _log / _flush_logs(message_log, config) | log_sink | Base-owned structured logging, shared by every class. Single-underscore, not name-mangled — see Section 3; these are defined on _PropagationMixin and called from BaseWorkflow, BaseAgent, and BaseToolService, so they can't be double-underscore without breaking cross-class access. |
...
| Access | Signature | Config / kwargs keys | Purpose |
| --- | --- | --- | --- |
| PUBLIC | async execute(payload, config=None, tenant_id=None, correlation_id=None, message_log=None, **kwargs) -> dict | timeout, retries, auth_token, max_latency | Template method. Validates → resolves endpoint → builds payload → checks the commit gate → HTTP POST. Measures wall-clock latency across the full call, logs a tool_execute_complete entry to message_log (when the caller passes one), and raises NFRViolationError if latency exceeds max_latency (default 5.0s here, looser than BaseAgent/BaseRAGPipeline's 2.0s since this measures a network round trip). Returns result dict. |
...
| PRIVATE | _commit_gate(key, failed_keys, config, **kwargs) -> bool | commit_gate_enabled | Base-owned. Called immediately before any save/commit/write. Returns False if key is already present in failed_keys — the item is excluded from commit and left for DLQ investigation while sibling items proceed. Single-underscore, not name-mangled — same reason as BaseWorkflow's _log/_flush_logs above (Section 2.4). |
...
>
> Both the token-budget check above and the latency SLO checks in `BaseAgent.run()`, `BaseRAGPipeline.retrieve()`, and `BaseToolService.execute()` (Sections 2.3, 2.5, 2.6) raise the same `NFRViolationError`. Unlike `DehydrationInterrupt`, it is not caught separately anywhere — it falls through the normal `except Exception` path in whichever method raised it, so a breach is a regular failure: added to `failed_keys`, handed to `_handle_error()`, and routed to the DLQ like any other exception.
...
class NFRViolationError(Exception):
    """Raised when a measured non-functional requirement — the latency SLO
    in BaseAgent.run() / BaseRAGPipeline.retrieve() / BaseToolService.execute(),
    or the token budget in BaseGovernanceFramework.enforce_governance_gate() —
    is breached. Unlike DehydrationInterrupt, this is not a special
    control-flow signal: it falls through the normal except Exception path,
    so a breach is added to failed_keys and routed to the DLQ like any
    other failure."""
...
class BaseGovernanceFramework(ABC):
    ...
    def __enforce_token_budget(self, state: Dict[str, Any], io_config: Dict[str, Any]) -> None:
        ...
        if current_usage >= max_budget:
            raise NFRViolationError(f"Token budget exhausted. "
...
class BaseAgent(ABC, _PropagationMixin):
    ...
    def run(self, state: Dict[str, Any], config: Dict[str, Any] = None,
        ...
        try:
            ...
            # FinOps — token usage tracking. Feeds BaseGovernanceFramework's
            # token-budget gate on the *next* call, and BaseObservability's
            # total_tokens metric on this one.
            ...
            # Latency SLO — measured over the full sequence above, logged
            # unconditionally (even on breach) so the telemetry record always
            # reflects what actually happened.
            latency = time.time() - start
            max_latency = kwargs.get('max_latency', config.get(
                'max_latency', float(os.getenv('MAX_LATENCY', 2.0))))
    ...
    # PRIVATE — framework owns token accounting entirely
    def __track_usage(self, messages, response, config=None, **kwargs):
        """Prefers an actual token count from the LLM provider's response
        metadata (response['metadata']['tokens_used']); falls back to a
        ~4-chars/token estimate over the outbound messages when a provider
...
class BaseWorkflow(ABC, _PropagationMixin):
    ...
    def execute(self, state: Dict[str, Any], config: Dict[str, Any] = None,
        ...
        try:
            ...
            self._flush_logs([{'timestamp': time.time(), 'level': 'INFO',
...
class BaseRAGPipeline(ABC, _PropagationMixin):
    ...
    def retrieve(self, query: str, config: Dict[str, Any] = None,
                 tenant_id: str = None, correlation_id: str = None,
                 message_log: List[dict] = None, **kwargs) -> List[dict]:
        config = config or {}
        start = time.time()
...
class BaseToolService(ABC, _PropagationMixin):
    ...
    async def execute(self, payload: Dict[str, Any],
                       ...
                       failed_keys: List[str] = None, correlation_id: str = None,
                       message_log: List[dict] = None, **kwargs) -> Dict[str, Any]:
        config = config or {}
        start = time.time()
        ...
        latency = time.time() - start
        max_latency = kwargs.get('max_latency', config.get(
            'max_latency', float(os.getenv('MAX_LATENCY', 5.0))))
...
| log_sink | LOG_SINK | 'stdout' | _flush_logs() |
| observability_engine | — (object reference, not env-resolvable) | None | BaseWorkflow.execute() — record_telemetry() sink; falls back to _flush_logs() if unset (Section 2.9.1) |
...
| max_token_budget | MAX_TOKEN_BUDGET | inf | BaseGovernanceFramework.enforce_governance_gate() — per-request ceiling checked against state['cumulative_tokens_used'], raises NFRViolationError on breach |
