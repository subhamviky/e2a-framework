...
| Access | Signature | Config / kwargs keys | Purpose |
| --- | --- | --- | --- |
| PUBLIC | execute(state, config=None, correlation_id=None, **kwargs) -> Dict | workflow_id, routing_strategy | Public entry point. Validates tenant_id, accepts correlation_id from the caller (BaseValidationService, normally) or originates one as a standalone fallback, resolves io_config/idempotency_key, builds and validates the workflow graph, resolves and invokes the target agent, flushes logs, and dispatches failed_keys to the DLQ. |
...
| INTERFACE | _collect_metrics(config, **kwargs) | namespace, dimensions | Emit custom metrics to CloudWatch/Datadog/GCP Monitoring. |
| INTERFACE | _collect_traces(config, **kwargs) | trace_id, service_name | Emit distributed traces to X-Ray/Jaeger/Cloud Trace. |
| INTERFACE | _collect_logs(config, **kwargs) | log_level, correlation_id | Structured log shipping. Called by the shared __flush_logs() helper. |
...
| INTERFACE | _apply_policy(payload, config, **kwargs) | policy_file, redaction_rules | Policy enforcement and PII redaction. |
| INTERFACE | _circuit_breaker(failures, config, **kwargs) | fail_max, reset_timeout | Circuit breaker state transitions + metric emission. |
