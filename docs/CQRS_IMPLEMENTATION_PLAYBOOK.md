...
class BaseGovernanceFramework(ABC):
    ...
    def __enforce_tenant_quota(self, state, io_config) -> None:
        ...
        if used >= max_quota:
            raise ValueError(
                f'Governance Violation: tenant request quota exhausted. '
...
class BaseCommandService(ABC, _PropagationMixin):
    ...
    def mutate(self, payload, config=None, **kwargs) -> dict:
        ...
        self._log(message_log, correlation_id, 'INFO', 'command_mutation_complete')
...
class BaseQueryService(ABC, _PropagationMixin):
    ...
    def fetch(self, query_params, config=None, **kwargs) -> dict:
        ...
        if cached_data:
            self._log(message_log, correlation_id, 'INFO', 'query_cache_hit')
...
`BaseOrchestrator.execute()` catches exceptions from either service uniformly and appends `idempotency_key` to `failed_keys`, exactly as `BaseWorkflow` does for `BaseAgent`. The one CQRS-specific branch is in the `finally` block's DLQ dispatch: `if failed_keys and not is_read`. A failed query never reaches `_send_to_dlq()` — see the Landing Zone doc's Section 6 for the reasoning. A concrete `_handle_error()` implementation should still surface a query failure to the caller as a normal error response; it simply does not enter the Saga's compensation flow.
